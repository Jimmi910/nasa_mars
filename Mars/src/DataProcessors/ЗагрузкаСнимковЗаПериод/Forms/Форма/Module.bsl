...
Процедура ЗагрузитьСнимкиЗаПериод(Команда)
	...
	Если Объект.ДатаНачала = ПустаяДата 
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не верно указан период'"), "Период",
			"Объект", , Истина);
		Возврат;
...
Процедура ОжиданиеЗавершенияЗагрузкиСнимков() Экспорт
	...
	Элементы.ИндикаторЗагрузки.Подсказка = "Обработано " + РезультатФоновойЗагрузки.Обработано + 
		" из " + РезультатФоновойЗагрузки.ВсегоСнимков;
